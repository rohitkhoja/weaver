...
from LLM import Call_OpenAI, Call_Gemini
...
def check_baselines(model, table, table_name, question):
    logging.basicConfig(filename=f'baselinelogs/{dataset}_{model.model[:8]}_ab_verifyPlan.log', level=logging.INFO,
...
def main(dataset, model_name):
    ...
    for index in range(n):
        ...
        table_id, table_name, table, question, answer = dataLoader.get_table()
        dataLoader.count += 1
        ...
        hybridqa = HybridQA(table_id, question, table, table_name, answer, dataset, model_name, model)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='wikitq',
                        choices=['wikitq', 'tabfact', 'tat', 'finqa', 'swan'])
